function Get-DriverDeductions {
    <#
    .SYNOPSIS
        Retrieves driver deduction records
    ...
    .PARAMETER Select
        ...
        Example: "driverDeductionId,driverId,amount"
    
    .PARAMETER OrderBy
        Optional. OData orderby expression.
        Example: "deductionDate desc"
    ...
    $uri = "$BaseUrl/driverDeductions$queryString"
    ...
    try {
        ...
        if ($PassThru) {
            ...
            if ($response.driverDeductions) {
                return $response.driverDeductions
            } else {
                return $response
            }
        }
