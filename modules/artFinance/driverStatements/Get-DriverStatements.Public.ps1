function Get-DriverStatements {
    <#
    .SYNOPSIS
        Retrieves driver statement records
    ...
    .PARAMETER Select
        ...
        Example: "driverStatementId,driverId,statementDate"
    
    .PARAMETER OrderBy
        Optional. OData orderby expression.
        Example: "statementDate desc"
    ...
    $uri = "$BaseUrl/driverStatements$queryString"
    ...
    try {
        ...
        if ($PassThru) {
            ...
            if ($response.driverStatements) {
                return $response.driverStatements
            } else {
                return $response
            }
        }
