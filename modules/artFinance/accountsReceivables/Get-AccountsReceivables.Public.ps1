function Get-AccountsReceivables {
    <#
    .SYNOPSIS
        Retrieves accounts receivable records
    ...
    $uri = "$BaseUrl/accountsReceivables$queryString"
    ...
    try {
        ...
        if ($PassThru) {
            ...
            if ($response.accountsReceivables) {
                return $response.accountsReceivables
            } else {
                return $response
            }
        }
